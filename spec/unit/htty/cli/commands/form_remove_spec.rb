...
RSpec.describe HTTY::CLI::Commands::FormRemove do
  describe 'class' do
    ...
    it 'should have the expected command_line' do
      expect(klass.command_line).to eq('form-remove')
    end
    ...
    it 'should have the expected help' do
      expect(klass.help).to eq("(Help for \e[1mform-remove\e[0m is not available)")
    end

    it 'should have the expected help_extended' do
      expect(klass.help_extended).to eq("(Help for \e[1mform-remove\e[0m is not " +
                                        'available).')
    end
